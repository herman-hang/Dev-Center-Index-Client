        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-tooltip :enterable="false" effect="dark" content="服务升级" placement="top">
              <el-button type="success" icon="el-icon-upload2" size="mini" @click="editDialog(scope.row.id)"></el-button>
            </el-tooltip>
  ...
  methods: {
    ...
    submitAdd() {
      ...
      this.$refs.addFormRef.validate(async valid => {
        if (!valid) return;
        const { data: res } = await this.$http.post('Pay/add', this.addForm);
        if (res.code === 200) {
          // 易支付通道
          ...
          switch (res.data.pay_type) {
            case 'wxpay':
              this.$router.push({ path: '/pay/wxpay', params: { data: res.data } });
              break;
            case 'qqpay':
              // 关闭加载图标
